...
In this section change the `<artifactId>` of `com.cleverpine.user-api` dependency to the same value as `<api-project.name>`. In our
...
* Finally, rename the package `com.cleverpine.user` to an appropriate one such as, for example: `com.cleverpine.user`
...
**Important!** If using 'decodingAuthTokenConfig' bean(default) from `com.cleverpine.user.config.ViravaHelperConfig` only the basic structure of the token is validated. It is expected that the validity and authenticity of the token is checked by an API Gateway
